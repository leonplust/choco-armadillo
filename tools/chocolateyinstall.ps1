...
$url        = '$toolsDir\armadillo.tar' # download url, HTTPS preferred
#$url64      = '' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url

$packageArgs = @{
  packageName   = $env:ChocolateyPackageName
  unzipLocation = $toolsDir
  fileType      = 'tar' #only one of these: exe, msi, msu
  url           = $url
  url64         = $url
  #file         = $fileLocation

  softwareName  = 'armadillo*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
...
## Install-ChocolateyZipPackage -PackageName 'armadillo' `
## -Url 'http://sourceforge.net/projects/arma/files/armadillo-9.200.7.tar.xz' `
## s-Url64 'http://sourceforge.net/projects/arma/files/armadillo-9.200.7.tar.xz' `
## -UnzipLocation "$toolsDir"
...
# $toolsDir = (Split-Path -parent $MyInvocation.MyCommand.Definition)
# Get-ChocolateyUnzip -FileFullPath "c:\armadillo.tar" -Destination $toolsDir
